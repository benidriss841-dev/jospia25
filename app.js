...
function getLastMatriculeId() {
    let maxId = 0;
    seminaristes.forEach(s => {
        if (s.matricule && s.matricule.startsWith('25-JOS')) {
            const num = parseInt(s.matricule.replace('25-JOS', ''), 10);
            if (!isNaN(num) && num > maxId) maxId = num;
        }
    });
    return maxId;
...
async function deleteAllData() {
    if (!confirm('ATTENTION: Vous êtes sur le point de TOUT supprimer.\nCeci est irréversible.\n\nVoulez-vous vraiment continuer ?')) return;

    const confirmation = prompt('Pour confirmer, tapez "SUPPRIMER" en majuscules :');
    if (confirmation !== 'SUPPRIMER') {
        return showToast('Suppression annulée (Code incorrect)', 'info');
    }
    ...
    if (USE_LOCAL_STORAGE) {
        ...
        try {
            const { error } = await supabaseClient.from('seminaristes').delete().neq('id', 0);
            if (error) throw error;
            seminaristes = [];
...
function refreshCurrentView() {
    const currentRoute = document.querySelector('.nav-link.active')?.dataset.route || 'dashboard';
    if (currentRoute === 'dashboard') renderDashboard();
    else if (currentRoute === 'all') renderList();
    else if (currentRoute === 'search') renderSearch();
    else if (currentRoute === 'add') { /* keep form */ }
    else if (currentRoute === 'levels') renderLevelsByGenre();
    else if (currentRoute === 'dortoirs') renderGroupList('dortoir', [...DORTOIRS_FRERES, ...DORTOIRS_SOEURS]);
    else if (currentRoute === 'harakas') {
        const allH = new Set([...seminaristes.map(s => s.halaqa).filter(Boolean), ...DORTOIRS_FRERES, ...DORTOIRS_SOEURS]);
        renderGroupList('halaqa', Array.from(allH));
    }
}
...
function subscribeToRealtime() {
    if (USE_LOCAL_STORAGE || !supabaseClient) return;
    ...
    supabaseClient
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'seminaristes' }, (payload) => {
            ...
            if (payload.eventType === 'INSERT') {
                seminaristes.push(payload.new);
                showToast(`Nouveau séminariste : ${payload.new.prenom} ${payload.new.nom}`);
            } else if (payload.eventType === 'UPDATE') {
                const index = seminaristes.findIndex(s => s.matricule === payload.new.matricule);
                if (index >= 0) seminaristes[index] = payload.new;
            } else if (payload.eventType === 'DELETE') {
                seminaristes = seminaristes.filter(s => s.matricule !== payload.old.matricule);
                showToast('Séminariste supprimé a distance', 'warning');
            }
            refreshCurrentView();
        })
        .subscribe((status) => {
            if (status === 'SUBSCRIBED') {
                ...
                showToast('Erreur connexion temps réel', 'error');
            }
        });
}
